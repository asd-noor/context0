...
func openStore(dir string) (*graph.Store, error) {
	store, err := graph.OpenReadOnly(gitRoot(dir), "codemap-ctx0.sqlite")
	if err != nil {
		return nil, err
	}
	return store, nil
...
func newSymbolCmd(projectDir *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runSymbol(*projectDir, args[0], lang, withSource, jsonOut)
		},
	}
...
func runSymbol(dir, name, lang string, withSource, jsonOut bool) error {
	root := gitRoot(dir)
	ctx := context.Background()
	store, err := openStore(dir)
	if err != nil {
